...
- **`ieri_oggi/models.py`** — SQLAlchemy 2.0 declarative models. `Person` (nome/cognome required, rest optional) and `Photo` (`categoria` ∈ {`giovane`,`recente`}, `is_cover`, `dati`, `thumb`, `mime`). `Person.foto` cascades delete-orphan. Helpers `cover()`, `cover_gallery()` (recent preferred), `foto_categoria()` drive templating. `make_engine()` enables SQLite `PRAGMA foreign_keys=ON`.
...
- **`ieri_oggi/templates/`** (`base`, `gallery`, `dettaglio`, `form`, `accedi`) + **`ieri_oggi/static/style.css`** — vanilla CSS, large fonts / tap targets, responsive grid.
